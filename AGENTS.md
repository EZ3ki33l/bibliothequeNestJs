...
- Tests backend Jest : unitaires sur services + `slugify` + `scheduleReview` (seuil 90 % via `collectCoverageFrom` incluant `common/**/*.ts`, Prisma mocké) ; e2e 401 admin et reviews (`GET /reviews/due`, `POST /reviews/:id/rate`) avec Prisma et `getSession` mockés (Prisma 7 WASM et better-auth ESM cassent Jest). Pas de tests frontend ni de perf.
